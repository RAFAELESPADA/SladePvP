...
public class Feast extends BukkitRunnable {
    ...
    private final List<ItemStack> items = Arrays.asList(
            new ItemBuilder("�bEspada de ferrp!", Material.IRON_FENCE)
                    .nbt("item", "glad")
                    .nbt("cancel-drop")
                    .toStack(),
            new ItemBuilder("�bPuxar!", Material.IRON_FENCE)
                    .nbt("item", "glad")
                    .nbt("cancel-drop")
                    .toStack(),new ItemBuilder("�bPuxar!", Material.IRON_FENCE)
                    .nbt("item", "glad")
                    .nbt("cancel-drop")
                    .toStack(),new ItemBuilder("�bPuxar!", Material.IRON_FENCE)
                    .nbt("item", "glad")
    ...
    public void run() {
        ...
        if (!spawned && minutes == 0) {
            if (seconds == 50 || seconds == 30 || seconds == 20 || seconds == 10 || seconds == 3) {
                Bukkit.broadcastMessage("§c§lFEAST: §fO feast irá spawn em " + seconds + " " + ((seconds > 1) ? "segundos" : "segundo"));
            }

            if (seconds <= 0) {
                Bukkit.broadcastMessage("§a§lO FEAST SPAWNOU!");
                // Duração do feast (em segundos)
