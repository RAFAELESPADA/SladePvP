...
public class OneVsOne extends WarpDuoBattleHandle {
	...
	public void onInteractEntity(PlayerInteractEntityEvent event) {
		...
		if (HelixCooldown.inCooldown(player.getName(), "1v1-challenge-" + target.getName())) {
			player.sendMessage("§e§l1V1 §eVocê jÃ¡ convidou este jogador recentemente");
			return;
		}
		
		HelixCooldown.create(player.getName(), "1v1-challenge-" + target.getName(), TimeUnit.SECONDS, 15);
		target.sendMessage("§e§l1V1 §eVocê foi convidado por " + (!FakeAPI.hasFake(player) ? player.getName() : FakeAPI.getNick(player)) + " para uma batalha");
	...
	public void onDeath(HelixPlayerDeathEvent event) {
		...
		loser.sendMessage("§cVocê perdeu a batalha contra " + (!FakeAPI.hasFake(winner) ? winner.getName() : FakeAPI.getNick(winner) + "§c."));
		...
		winner.sendMessage("§aVocê ganhou a batalha contra " + ((!FakeAPI.hasFake(loser) ? loser.getName() : FakeAPI.getNick(loser) + " §7(" + (event.isValidKill() ? "Conta" : "NÃ£o conta") + ")")));
	...
	public void sendBattleItems(Player player) {
		...
		player.getInventory().setItem(0, new ItemBuilder("§fEspada de Diamante", Material.DIAMOND_SWORD)
				.nbt("cancel-drop").addEnchant(org.bukkit.enchantments.Enchantment.DAMAGE_ALL, 2)
				.toStack()
		);
