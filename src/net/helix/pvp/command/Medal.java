...
public class Medal implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("medalha")) {
            ...
            if (medal == null) {
                player.sendMessage("§cNao foi possivel encontrar a medalha se" + args[0] + "�c.");
                return true;
            }
            if (!player.hasPermission("medal." + medal.getName().toLowerCase())) {
                player.sendMessage("§cVoce nao tem permissao para utilizar essa medalha.");
                return true;
            }
            player.sendMessage("§aVoce selecionou a medalha " + medal.getMedal());
        ...
        if (command.getName().equalsIgnoreCase("medalhas")) {
            ...
            medals.add(new TextComponent("§aSuas medalhas sao: "));
            Medals[] values;
